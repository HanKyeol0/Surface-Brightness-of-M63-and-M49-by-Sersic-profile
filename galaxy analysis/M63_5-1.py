...
NAME = 'M63'
imglist = list(DATADIR.glob('M63_[IR].fits'))
imglist.sort()
print(imglist)
...
class SloanGalaxySurfacePhot:
    ...
    def ellipse(self, display=True, backgr=True, minsma=0.1, maxsma=1000.,
    ...
    def apphot_stars(self, display=True, vmax=None):
...
#                    linestyle='--', color='gray', lw=1)
    ...
    def fit_radial_profile(self, mask=None, double=False):
        ...
        popt, pcov = curve_fit(func, self.r_sma[self.r_sma > 20], self.mu[self.r_sma > 20], p0=p0,
                               sigma=self.mu_err[self.r_sma > 20], bounds=bounds)
        perr = np.sqrt(np.diag(pcov))
        self.rmse = np.std(self.mu[self.r_sma > 20] - func(self.r_sma[self.r_sma > 20], *popt))
        self.popt, self.perr, self.fitfunc = popt, perr, func
        
    def background_subtraction(self, box=30, filt=3, show=True):
        mask = make_source_mask(self.img, nsigma=2, npixels=3, dilate_size=11)
        self.img = self.img.byteswap().newbyteorder() # new byte order
        bkg_sep = sep.Background(self.img, 
                                 mask=mask, bw=box, bh=box, fw=filt, fh=filt)
        bkgsub_sep = self.img - bkg_sep.back()
        ...
        if show:
            ...
            data2plot = [
                dict(ax=axs[0, 0], arr=self.img,                      title="Original data"),
...
# fetch image
m89_g.set_image(vmax=0.7)

m89_g.background_subtraction(box=700)

# query stars from SDSS to check the zeropoint
m89_g.set_ref_stars(maglimit=15.2, display=True, vmax=0.7)

m89_g.apphot_stars(vmax=0.7)
...
m89_g.ellipse(minsma=0.1, maxsma=500., vmax=0.7)
...
m89_r = SloanGalaxySurfacePhot(imgpath, sky_r0=700, pixel_scale=pixel_scale,
                               name='M63', band='r')

# fetch image
m89_r.set_image(vmax=0.7)

m89_r.background_subtraction(box=700)

# query stars from SDSS to check the zeropoint
m89_r.set_ref_stars(maglimit=15.2, display=True, vmax=0.7)

m89_r.apphot_stars(vmax=0.7)
mag0_from_apphot_r = m89_r.get_mag0()

mag0_from_apphot_r

print(np.mean(mag0_from_apphot_r))

m89_r.fwhm

m89_r.set_kwargs_simple()  # set keyword arguments with default setting
                           # use set_kwargs function for custom kwargs.
m89_r.ellipse(minsma=0.1, maxsma=500., vmax=0.7)
...
# color profile
display_color_profile(m89_g, m89_r, 'M63')
