...
class SloanGalaxySurfacePhot:
    ...
    def ellipse(self, display=True, backgr=True, minsma=0.1, maxsma=200.,
...
# SDSS pixel scale
pixel_scale = 0.388   # arcsec/pixel (SDSS image)
...
m89_g.set_ref_stars(maglimit=15.2, display=True)
...
m89_g.ellipse(minsma=0.1, maxsma=300., vmax=0.7)
...
# query stars from SDSS to check the zeropoint
m89_r.set_ref_stars(maglimit=15.2, display=True)

m89_r.apphot_stars()

mag0_from_apphot_r = m89_r.get_mag0()

print(mag0_from_apphot_r)
...
m89_r.set_kwargs_simple()  # set keyword arguments with default setting
                           # use set_kwargs function for custom kwargs.
m89_r.ellipse(minsma=0.1, maxsma=300., vmax=0.7)
